...
> = ({ children, hoveredNode }) => {
  ...
  useEffect(() => {
    ...
    setSettings({
      defaultDrawNodeLabel: drawLabel, // 기본 노드 라벨 그리기 함수 설정
      defaultDrawNodeHover: drawHover, // 기본 노드 호버 그리기 함수 설정
      nodeReducer: (node: string, data: Attributes) => {
        ...
        if (debouncedHoveredNode) {
          ...
          const isNeighborNode =
            ...
            graph.hasEdge(debouncedHoveredNode, node); // 현재 노드가 이웃 노드인지 확인
  ...
  }, [sigma, graph, debouncedHoveredNode]); // 의존성 배열에 sigma, graph, debouncedHoveredNode 포함
  ...
  }, [debouncedHoveredNode]); // 의존성 배열에 debouncedHoveredNode 포함
